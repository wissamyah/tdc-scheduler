...
export default function DeleteMemberModal({ isOpen, onConfirm, onCancel, memberUsername }) {
  const { t } = useLanguage();
  ...
  const handleSubmit = (e) => {
    ...
    if (password !== 'R5') {
      setError(t('deleteMemberModal.incorrectPassword'));
      return;
    }
        ...
        <div className="flex items-center justify-between p-6 border-b border-creed-danger/30">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-display font-bold text-creed-danger uppercase tracking-wide">
              {t('deleteMemberModal.criticalWarning')}
            </h2>
          </div>
          <button
            onClick={handleCancel}
        ...
        <div className="p-6">
          <div className="bg-creed-danger/10 border border-creed-danger/30 rounded-lg p-4 mb-6">
            <p className="text-creed-text font-body text-center">
              {t('deleteMemberModal.youAreAboutToDelete')} <span className="font-bold text-creed-danger">{memberUsername}</span> {t('deleteMemberModal.fromRoster')}
            </p>
            <p className="text-creed-danger font-display font-bold text-center mt-2 uppercase tracking-wide">
              {t('deleteMemberModal.cannotBeUndone')}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label
                htmlFor="delete-member-password"
                className="block text-sm font-display font-semibold text-creed-text mb-2 uppercase tracking-wide"
              >
                {t('deleteMemberModal.enterPasswordToConfirm')}
              </label>
              <input
                type="text"
                id="delete-member-password"
                value={password}
                onChange={(e) => {
                ...
                placeholder={t('deleteMemberModal.enterPasswordPlaceholder')}
                autoFocus
              />
              {error && (
              ...
              <p className="text-creed-muted text-xs font-body mt-2">
                {t('deleteMemberModal.typeRequired')}
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                ...
                {t('deleteMemberModal.deleteMember')}
              </button>
            </div>
          </form>
        </div>
